...
{
public:
	_XMLCFGFile_impl() :xml_file_(nullptr)
	{}
	~_XMLCFGFile_impl()
	{
	...
	{
		ofstream ofs(xml_file_name, std::ios_base::out | std::ios_base::trunc);
		if (ofs)
		{
			ofs << "<root></root>";
			ofs.close();
		...
		{
			throw std::runtime_error(ERR_WHERE "创建文件失败 filename = " + std::string(xml_file_name));
		}
	}
...
private:
	...
	xml_node<char> *GetNode(const char *path)
	{
		auto tmp = Parse(path);
		xml_node<char> *node = xml_.first_node("root");
		if (nullptr == node)throw logic_error("xml文件不存在root结点");
...
void XMLCfgFile::Clear()
{
	impl_->Clear();
}
void XMLCfgFile::Create(const char *xml_file_name)
{
	impl_->Create(xml_file_name);
}
