...
LogFileBase *Logger::FindLogFileByName(const char *name)
{
...
void Logger::ReflushLowerLogLevel()
{
	if (logfiles_.empty())return;
	auto it = min_element(logfiles_.begin(), logfiles_.end(), [](const unique_ptr<LogFileBase> & x, const unique_ptr<LogFileBase> & y){
		return static_cast<unsigned int>(x->Level()) < static_cast<unsigned int>(y->Level());
	});
	log_type_ = (*it)->Level();
}
